...
time_format(decolagem, '%H:%i') decolagem, time_format(pouso, '%H:%i') pouso,
time_format(corte, '%H:%i') corte, combustivel_decolagem, combustivel_pouso, 
combustivel_consumido, ias, tas, gs, n1, egt, hp, torque, volts, 
amperes, fuel_press, oil_press, oil_temp, fueld_flow, oat, carga_transportada, combustivel_abastecido, ce, fornecedor from voos where id = {$id}");
$sth->execute();
$voo = $sth->fetchAll();
